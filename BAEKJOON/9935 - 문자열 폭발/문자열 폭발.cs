...
class Program
{
    static string ExplodeString(string s, string bomb)
    {
        // 폭발 문자열을 처리하는 함수 정의
        ...
        foreach (char c in s)
        {
            ...
            if (stack.Count >= bombLen)
            {
                ...
                {
                    if (stack[stack.Count - bombLen + i] != bomb[i])
                    {
                        match = false;
                        break;
                    }
                ...
                if (match)
                {
                    ...
                    stack.RemoveRange(stack.Count - bombLen, bombLen);
                }
            }
        }
    ...
    static void Main()
    {
        ...
        // 결과 출력
        Console.WriteLine(ExplodeString(s, bomb));
